...
public class Texture {
    ...
    public Texture(TextureData data) {
        logger.info("Loaded texture: " + data.path);
        logger.info("Texture size: " + data.width + "x" + data.height);
        if (data.width != data.height) {
            logger.warn("Texture size is uneven!");
        } else {
            if (!Mathf.isPowerOfTwo(data.width)) {
                logger.warn("Texture size is not in power of 2!");
            }
        }
        this.id = data.id;
    ...
    private static TextureData loadTexture(String fileName, boolean inResources) {
        ...
        if (inResources) {
            ...
            try {
                ...
                data = new TextureData(textureId, fileName, image.getWidth(), image.getHeight());
            } catch (IOException e) {
                e.printStackTrace();
            }
        } else {
